...
struct TreeConnectableTests {
    ...
    /// The rank for the goal on `db` marks `api` and the flow that joins
    /// them, puts them first, and leaves `cache` and its flow unmarked in
    /// model order after them.
    @Test func theSidebarMarksWhatTheSelectedNodeReachesAndPutsItFirst() async throws {
        ...
        canvas.select(goalId, addingToSelection: false)
        ...
        #expect(Set(marked) == ["component:db", "component:api", "flow:api->db"])
        #expect(rows.firstIndex { $0.isConnectable == false } == marked.count)
        #expect(rows.count == elements.count)
        #expect(rows.filter { $0.isConnectable == false }.map(\.element.payload)
            == ["component:cache", "flow:api->cache"])

        let counts = rows.filter(\.isConnectable).map(\.element.threats.count)
        #expect(counts == counts.sorted(by: >))
    }

    /// A node on `api` reaches every element of the shop, so every row is
    /// marked and none is dimmed.
    @Test func aNodeThatReachesEveryElementMarksEveryRow() async throws {
        let (editor, _, _, elements, goalId) = try await anEditorWithAGoal()
        ...
        #expect(rows.count == elements.count)
        #expect(rows.contains { $0.isConnectable == false } == false)
        let counts = rows.map(\.element.threats.count)
        #expect(counts == counts.sorted(by: >))
    }
    ...
    @Test func theSearchReadsTheQueryAndOffersEveryElementOnRequest() async throws {
        ...
        // `api` and `cache` share the name the technology gives them, so
        // the query names the id, which the search reads as well.
        #expect(TreeConnectable.search(elements, from: "component:db", query: cache.sourceId, everyElement: false).isEmpty)

        let every = TreeConnectable.search(elements, from: "component:db", query: cache.sourceId, everyElement: true)
        #expect(Set(every.map(\.payload)) == [cache.payload, "flow:api->cache"])

        let unjoined = TreeConnectable.search(elements, from: nil, query: "", everyElement: false)
        #expect(Set(unjoined.map(\.payload)) == Set(elements.map(\.payload)))
    }
    ...
    @Test func theMenuOnAFilledBoxOffersItsThreatsThenDelete() async throws {
        ...
        // A box joins the way a step does, so the join rows come first.
        #expect(titles == ["Join to\u{2026}"] + api.threats.map(\.name) + ["Delete"])
