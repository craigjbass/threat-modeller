...
enum SystemSheetKind: String, CaseIterable, Identifiable, Sendable {
    ...
    func count(in session: ThreatModelSession) -> Int {
        ...
        case .assets: return session.canvas.systemAssets.count
        case .thirdParties: return session.canvas.thirdParties.count
        case .useCases: return session.canvas.useCases.count
