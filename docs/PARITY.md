...
| `.controls` | `mitigated_by` on a `control` block | The label names a `mitigates` edge the `.arch` file declares, written `<protector>-><protected>`. A control may hold one block per edge, and each block states what that edge takes off. `ApplyControlAnswers` drops a mapping whose edge the architecture no longer declares, or whose edge answers another threat or another element, keeps the control's status, and warns; the next compile writes no `mitigated_by`. The threat card draws one row per edge that protects the element, with a `control-mitigated-by-<key>-<edge>` tick that writes or removes the block and a `control-mitigated-by-percent-<key>-<edge>` stepper beside a ticked one. |
